...
extension UIView {
    ...
    func viewAnimation(with duration: Double) {
        UIView.transition(with: self, duration: duration, options: .transitionCrossDissolve, animations: {
            self.isHidden = false
        })
    }
    
    func addColorToViewAndDecolorizetoViews(views: [UIView], color: UIColor, decolorize: UIColor = .clear) {
        for view in views {
            view.backgroundColor = view == self ? color : decolorize
            self.viewAnimation(with: 0.5)
        }
    }
...
extension UILabel {
    ...
    func addColorToLabelAndDecolorizetoLabels(labels: [UILabel], color: UIColor, decolorize: UIColor = .white) {
        for label in labels {
            label.textColor = label == self ? color : decolorize
        }
    }
}
...
extension NSAttributedString {
    ...
    static func setGreenOrRedColor(value: String, changePercentage: Double) -> NSAttributedString {
        ...
        let attributedString = NSAttributedString(string: value, attributes: attribute)
