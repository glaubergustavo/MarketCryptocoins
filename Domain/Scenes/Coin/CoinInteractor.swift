...
class CoinInteractor: CoinBusinessLogic, CoinDataStore {
    ...
    func doFetchMarketChart(request: Coin.FetchMarketChart.Request) {
        ...
        marketChartDataWoker?.doFetchMarketChart(id: coinId, baseCoin: baseCoin, from: from, to: to, completion: { result in
            switch result {
    ...
    func doFetchOhlc(request: Coin.FetchOhlc.Request) {
        ...
        ohlcWorker?.doFetchOhlc(id: coinId, baseCoin: baseCoin, of: day, completion: { result in
            switch result {
    ...
    private func createCurrentDataResponse(baseCoin: CoinsFilterEnum, currentData: CurrentDataModel?) {
        if let currentData {
            let response = Coin.FetchCurrentData.Response(
                ...
                priceChangePercentageOneDay: currentData.marketData.priceChangePercentage24HInCurrency[baseCoin.rawValue] ?? 0.0, priceChangePercentageOneWeek: currentData.marketData.priceChangePercentage7DInCurrency[baseCoin.rawValue] ?? 0.0,
                ...
                higth24H: currentData.marketData.high24H[baseCoin.rawValue] ?? 0.0,
    ...
    private func createOhlcResponse(graphicData: [GraphicDataModel]?) {
        if let graphicData {
            let response = graphicData.map { data in
                return Coin.FetchOhlc.Response(
                    price: data[4],
                    dataEntry: ChartDataEntry(x: data[0], y: data[4])
                )
            }
            presenter?.presentOhlc(response: response)
        } else {
            presenter?.presentError(error: .undefinedError)
        }
    }
